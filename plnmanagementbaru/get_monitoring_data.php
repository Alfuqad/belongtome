...
$jam_nyala_min = isset($_POST['jam_nyala_min']) && $_POST['jam_nyala_min'] !== '' ? floatval($_POST['jam_nyala_min']) : 0;
$jam_nyala_max = isset($_POST['jam_nyala_max']) && $_POST['jam_nyala_max'] !== '' ? floatval($_POST['jam_nyala_max']) : 1000;
...
if ($jam_nyala_min !== 0 || $jam_nyala_max !== 1000) {
...
$where
GROUP BY f.IDPEL
HAVING (CASE WHEN MAX(f.DAYA) > 0 THEN SUM(f.PEMKWH)/(MAX(f.DAYA)/1000) ELSE 0 END) >= $jam_nyala_min
   AND (CASE WHEN MAX(f.DAYA) > 0 THEN SUM(f.PEMKWH)/(MAX(f.DAYA)/1000) ELSE 0 END) <= $jam_nyala_max
ORDER BY f.IDPEL
LIMIT $start, $length";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $rppenj = number_format($row['RPPENJ'], 0, ',', '.');
    ...
    $data[] = array(
        ...
        $row['TARIP'],           // 2: TARIP
        $row['DAYA'],            // 3: DAYA
        $jam_nyala,              // 4: Jam Nyala
        $pemkwh,                 // 5: PEMKWH
        $rppenj,                 // 6: RPPENJ
        $row['TGLBAYAR'],        // 7: TGLBAYAR
        $total_pembelian         // 8: Total Pembelian
    );
}
