    ...
    <body>
        ...
        <script>
            ...
            function uploadFile() {
                ...
                formData.append("file", fileInput.files[0]); // Menggunakan ID yang benar
                formData.append("folder", "nama_folder"); // Sesuaikan folder jika perlu
                ...
                fetch("http://localhost/plnmanagement/backend/upload.php", {
                    ...
                    .then(text => {
                        try {
                            let data = JSON.parse(text); // Coba parse ke JSON
                            if (data.status === "error") {
                                throw new Error(data.message);
                            }
                            alert("Upload berhasil: " + data.message);
                        } catch (error) {
                            console.error("Response bukan JSON:", text); // Log isi response untuk debugging
                            alert("Terjadi kesalahan: Server tidak mengembalikan JSON.");
                        }
                    })
                    .catch(error => {
